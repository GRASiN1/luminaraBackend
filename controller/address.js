...
async function handleCreateAddress(req, res) {
  const { fullName, email, phoneNumber, fullAddress, city, state, pinCode } =
    req.body;
...
async function handleGetAllAddressOfUser(req, res) {
  const addressList = await Address.find({ addressOf: req.user._id });
  if (addressList.length !== 0) {
    return res.status(200).json(addressList);
  } else {
    return res.status(200).json({ message: "user have no address" });
  }
}
